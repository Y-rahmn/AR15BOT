...
const grupch1 = 'https://chat.whatsapp.com/D gu6nsgzvfgw37vrjixuphp'; // OFFICIAL GRUP LU 1
...
conn.on('qr', qr =>
{
   ...
   conn.log(`[ ${moment().format("HH:mm:ss")} ] Scan kode qr mu cok!`);
});

conn.on('credentials-updated', () =>
{
   // save credentials whenever updated
   conn.log(`credentials updated!`)
...
conn.on('user-presence-update', json => conn.log(`[ ${moment().format("HH:mm:ss")} ] => bot by @_sadboy.ig`))
conn.on('message-status-update', json =>
{
   const participant = json.participant ? ' (' + json.participant + ')' : '' // participant exists when the message is from a group
   conn.log(`[ ${moment().format("HH:mm:ss")} ] => bot by @_sadboy.ig`)
})
...
conn.on('message-new', async(m) =>
{
   ...
   conn.log(`[ ${moment().format("HH:mm:ss")} ] => Nomor: [ ${id.split("@s.whatsapp.net")[0]} ] => ${text}`);
...
conn.log ("created group with id: " + group.gid)
...
conn.sendMessage(id, '3aalaikumsalam, ada apa kak? ketik !help untuk melihat menu bot euy...' ,MessageType.text);
}
else if (text == 'salam'){
conn.sendMessage(id, 'Waalaikumsalam, ada apa kak? ketik !help untuk melihat menu bot euy...' ,MessageType.text);
}
...
conn.sendMessage(id, 'Ya?, ada apa kak? ketik !help untuk melihat menu bot euy...' ,MessageType.text);
}
else if (text == 'P'){
conn.sendMessage(id, 'Ya?, ada apa kak? ketik !help untuk melihat menu bot euy...' ,MessageType.text);
}
...
conn.sendMessage(id, 'Cari Aris kah?..nih Aris wa.me/+6285712632324' ,MessageType.text);
}
else if (text == 'Bro'){
conn.sendMessage(id, 'Ya?, ada apa kak? ketik !help untuk melihat menu bot euy...' ,MessageType.text);
}
 ...
 if (text.includes("!yt"))
   ...
   if (text.includes("!ptl cewek"))
   {
	...
	var buf = Buffer.from(response, 'base64'); // Ta-da	
        ...
        .catch(
            (error) => {
                conn.log(error); // Logs an error if there was one
            }
        )
